...
import { read, diffDays, isAlreadyBooked, checkout } from "../actions/hotel";
...
const ViewHotel = ({ match, history }) => {
  ...
  const handleClick = async (e) => {
    ...
    let id= match.params.hotelId;
    console.log(id);
    console.log("Auth",auth.user._id)
    console.log(auth.token, match.params.hotelId);
    //setAlreadyBooked(true)
    
    let resp = await checkout(auth.token,auth.user._id , match.params.hotelId)
